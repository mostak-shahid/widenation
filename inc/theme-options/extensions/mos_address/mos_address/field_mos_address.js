(function( $ ) {
    ...
    redux.field_objects.mos_address = redux.field_objects.mos_address || {};

    $( document ).ready(
        function() {
            //redux.field_objects.mos_address.init();
        }
    );

    redux.field_objects.mos_address.init = function( selector ) {

        if ( !selector ) {
            selector = $( document ).find( ".redux-group-tab:visible" ).find( '.redux-container-mos_address' );
        }
        ...
        $( selector ).each(
            function() {
                ...
                if ( parent.hasClass( 'redux-container-mos_address' ) ) {
                ...
                el.find( '.redux-slides-remove' ).live(
                    'click', function() {
                        ...
                        var slideCount = $( this ).parents( '.redux-container-mos_address:first' ).find( '.redux-slides-accordion-group' ).length;
                        ...
                        if ( slideCount > 1 ) {
                            ...
                            $( this ).parents( '.redux-container-mos_address:first' ).find( '.redux-slides-accordion-group:last' ).find( '.redux-slides-header' ).text( content_new_title );
                        }
                    }
                );
