(function( $ ) {
    ...
    redux.field_objects.mos_service_slides = redux.field_objects.mos_service_slides || {};

    $( document ).ready(
        function() {
            //redux.field_objects.mos_service_slides.init();

        }
    );

    redux.field_objects.mos_service_slides.init = function( selector ) {

        if ( !selector ) {
            selector = $( document ).find( ".redux-group-tab:visible" ).find( '.redux-container-mos_service_slides' );
        }
        ...
        $( selector ).each(
            function() {
                ...
                if ( parent.hasClass( 'redux-container-mos_service_slides' ) ) {
                ...
                el.find( '.redux-slides-remove' ).live(
                    'click', function() {
                        ...
                        var slideCount = $( this ).parents( '.redux-container-mos_service_slides:first' ).find( '.redux-slides-accordion-group' ).length;
                        ...
                        if ( slideCount > 1 ) {
                            ...
                            $( this ).parents( '.redux-container-mos_service_slides:first' ).find( '.redux-slides-accordion-group:last' ).find( '.redux-slides-header' ).text( content_new_title );
                        }
                    }
                );

                el.find( '.mos_redux-slides-add' ).click(
                    function() {
                        ...
                        $( newSlide ).find( '.screenshot-photo' ).addClass( 'hide' );
                        $( newSlide ).find( '.screenshot-photo a' ).attr( 'href', '' );
                        $( newSlide ).find( '.remove-photo' ).addClass( 'hide' );
                        $( newSlide ).find( '.redux-option-photo' ).attr( 'src', '' );
