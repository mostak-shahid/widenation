(function( $ ) {
    ...
    redux.field_objects.mos_person = redux.field_objects.mos_person || {};

    $( document ).ready(
        function() {
            //redux.field_objects.mos_person.init();
        }
    );

    redux.field_objects.mos_person.init = function( selector ) {

        if ( !selector ) {
            selector = $( document ).find( ".redux-group-tab:visible" ).find( '.redux-container-mos_person' );
        }
        ...
        $( selector ).each(
            function() {
                ...
                if ( parent.hasClass( 'redux-container-mos_person' ) ) {
                ...
                el.find( '.redux-slides-remove' ).live(
                    'click', function() {
                        ...
                        var slideCount = $( this ).parents( '.redux-container-mos_person:first' ).find( '.redux-slides-accordion-group' ).length;
                        ...
                        if ( slideCount > 1 ) {
                            ...
                            $( this ).parents( '.redux-container-mos_person:first' ).find( '.redux-slides-accordion-group:last' ).find( '.redux-slides-header' ).text( content_new_title );
                        }
                    }
                );
